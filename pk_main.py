...
def vfill_temperature_method(dft, TcNames, volFracts = sensorVolFractHeights, vol = tankVol):
    ...
    Returns:
        TYPE: Panda Series. The calculated fill temperature.
...
df["dVFillVol"] =  df['CFMPStoOut'] - df['CHP12In'] # calculates change in fill of cold water. using out/in top of tank. 
df["VFillVol"] = df["dVFillVol"].cumsum() # fill assuming start at 0 fill. 
df["VFillVol0ed"] = df['dVFillVol']

'''
dVFillVoledFract held to zero even when more energy is put into the tank 
than starting point of full tank
'''

for ii in range(1, len(df['dVFillVol'])): # prevents drop below 0 fill. 
...
fig.write_html("Outputs/Temperature_with_Time.html")
...
fig.add_scatter(x=df['Minutes'],y=df["FillTemp6Odd"], name="FillTemp6Odd", line = dict(dash="dash"))
fig.add_scatter(x=df['Minutes'],y=df["FillTemp6Even"], name="FillTemp6Even", line = dict(dash="dash"))
fig.write_html("Outputs/Smoothed_Temperature_with_Time.html")
...
#plot(fig)
fig.write_html("Outputs/VFill_with_time.html")
...
xdata = []
ydata = []
for step in np.arange(firstInd, len(df)):
...
fig.write_html("Outputs/ScatterPlotWith_"+vfill+".html")
